...
sed -i "/uci commit network/i\uci set network.lan.broadcast='10.10.10.1'" $ZZZ         # IPv4 广播
...
sed -i "/uci commit system/i\uci set system.@system[0].hostname='yufeng'" $ZZZ           # 修改主机名称为yufeng-123

sed -i "s/OpenWrt /yufeng build $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" $ZZZ           # 增加自己个性名称
